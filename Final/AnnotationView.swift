    ...
    var body: some View {
        VStack{
            ...
            Button(action: {
                ...
                if(distance<500){
                    ref.child("user").child(Auth.auth().currentUser?.uid ?? "").child("annotations").child(location.id).observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    ref.child("user").child(Auth.auth().currentUser?.uid ?? "").child("annotations").child(location.id).removeValue()
...
struct AnnotationView_Previews: PreviewProvider {
    static var previews: some View {
        AnnotationView(location: Location(id:"",name: "d", coordinate: CLLocationCoordinate2D(
            latitude: 10,
            longitude: 10), type: "Random"))
    }
